module Day10 exposing (Particle, findMostCompactArrangement, findSize, inputText, maybeNegativeInt, parseParticleList, part1, part2, particleListParser, particleParser, stepByVelocity, testText, visualizeParticles)
...
particleParser =
    Parser.succeed Particle
        |. Parser.symbol "position=<"
        |. Parser.spaces
        |= maybeNegativeInt
        |. Parser.symbol ","
        |. Parser.spaces
        |= maybeNegativeInt
        |. Parser.symbol "> velocity=<"
        |. Parser.spaces
        |= maybeNegativeInt
        |. Parser.symbol ","
        |. Parser.spaces
...
maybeNegativeInt : Parser Int
maybeNegativeInt =
    Parser.oneOf
        [ Parser.int
        , Parser.succeed (\x -> x * -1)
            |. Parser.symbol "-"
            |= Parser.int
        ]


particleListParser : Parser (List Particle)
particleListParser =
    Parser.sequence
        { start = ""
        , end = ""
...
parseParticleList : String -> List Particle
parseParticleList text =
    Parser.run particleListParser text |> Result.withDefault []
...
findMostCompactArrangement : State -> State
findMostCompactArrangement ( stepsSoFar, particles ) =
    let
        newParticles =
            List.map stepByVelocity particles

        oldSize =
            findSize particles

        newSize =
            findSize newParticles
    in
    if newSize > oldSize then
        ( stepsSoFar, particles )

    else
        findMostCompactArrangement ( stepsSoFar + 1, newParticles )


stepByVelocity : Particle -> Particle
stepByVelocity particle =
    { particle
...
findSize : List Particle -> Int
findSize particles =
    let
        xs =
            List.map .x particles
...
visualizeParticles : List Particle -> String
visualizeParticles particles =
    let
        ...
        particleDict =
            Dict.Extra.fromListBy (\particle -> ( particle.x, particle.y )) particles

        xCoords =
            List.range minX maxX

        makeRow y =
            xCoords |> List.map (makePoint y) |> String.join ""

        makePoint y x =
            case Dict.get ( x, y ) particleDict of
                Just _ ->
                    "#"
...
part1 =
    parseParticleList
        >> toState
        >> findMostCompactArrangement
        >> Tuple.second
        >> visualizeParticles


part2 =
    parseParticleList >> toState >> findMostCompactArrangement >> Tuple.first
