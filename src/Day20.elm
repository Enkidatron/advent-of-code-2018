module Day20 exposing (Direction(..), PathSequence, PathTree(..), compressPath, directionListParser, directionParser, exampleText1, exampleText2, exampleText3, exampleText4, findRoomDistances, findRoomDistancesAlongPath, findRoomDistancesPathTree, getPathTreeLengths, groupParser, inputText, isDirectionChar, oppositeDirections, part1, part1Parser, part2, pathSequenceParser, pathTreeParser, scorePart1)

import Parser exposing ((|.), (|=), Parser)
import Set exposing (Set)


pathTreeParser : Parser PathTree
pathTreeParser =
    Parser.oneOf
        [ Parser.succeed Group
            |= groupParser
        , Parser.succeed Straight
            |= directionParser
            |= directionListParser
        ]


isDirectionChar : Char -> Bool
...
groupParser : Parser (List PathSequence)
groupParser =
    Parser.sequence
        { start = "("
        , end = ")"
        ...
        , item = Parser.lazy (\_ -> pathSequenceParser)
        , trailing = Parser.Forbidden
        }


pathSequenceParser : Parser PathSequence
pathSequenceParser =
    Parser.sequence
        { start = ""
        , end = ""
        , separator = ""
        , spaces = Parser.succeed ()
        , item = Parser.lazy (\_ -> pathTreeParser)
        , trailing = Parser.Forbidden
        }
...
directionListParser : Parser (List Direction)
directionListParser =
    Parser.sequence
        { start = ""
        , end = ""
...
type PathTree
    = Straight Direction (List Direction)
    | Group (List PathSequence)
...
type alias PathSequence =
    List PathTree


part1 text =
    Parser.run part1Parser text |> Result.map (scorePart1 0)


scorePart1 : Int -> PathSequence -> Int
scorePart1 prevScore pathSequence =
    case pathSequence of
        [] ->
            prevScore

        pathTree :: [] ->
            getPathTreeLengths pathTree |> List.maximum |> Maybe.withDefault 0 |> (\x -> x + prevScore) |> Debug.log "last pathTree"

        pathTree :: rest ->
            scorePart1 (prevScore + (getPathTreeLengths pathTree |> List.maximum |> Maybe.withDefault 0)) rest


getPathTreeLengths : PathTree -> List Int
getPathTreeLengths pathTree =
    case pathTree of
        Straight dir dirList ->
            [ List.length (compressPath [ dir ] dirList) ] |> Debug.log "scoring Straight"

        Group sequences ->
            List.map (scorePart1 0) sequences |> Debug.log "scoring Group"


compressPath : List Direction -> List Direction -> List Direction
compressPath stack pathToGo =
    case pathToGo of
        [] ->
            List.reverse stack
        ...
        dir :: rest ->
            case stack of
                [] ->
                    compressPath [ dir ] rest

                stackTop :: stackRest ->
                    if oppositeDirections dir stackTop then
                        compressPath stackRest rest

                    else
                        compressPath (dir :: stack) rest
...
part2 distance text =
    Parser.run part1Parser text
        |> Result.withDefault []
        |> findRoomDistances [] Set.empty
        |> Tuple.second
        |> Set.toList
        |> List.map String.length
        |> List.filter (\x -> x >= distance)
        |> List.length


findRoomDistances : List Direction -> Set String -> PathSequence -> ( List Direction, Set String )
findRoomDistances dirStack roomsSoFar sequence =
    case sequence of
        [] ->
            ( dirStack, roomsSoFar )
        ...
        pathTree :: rest ->
            let
                ( newDirStack, foundRooms ) =
                    findRoomDistancesPathTree dirStack pathTree
            in
            findRoomDistances newDirStack (Set.union roomsSoFar foundRooms) rest


findRoomDistancesPathTree : List Direction -> PathTree -> ( List Direction, Set String )
findRoomDistancesPathTree dirStack pathTree =
    case pathTree of
        Straight dir dirList ->
            findRoomDistancesAlongPath Set.empty dirStack (dir :: dirList)

        Group sequences ->
            let
                results =
                    List.map (findRoomDistances dirStack Set.empty) sequences
            in
            ( List.map Tuple.first results |> List.head |> Maybe.withDefault dirStack
            , List.map Tuple.second results |> List.foldl Set.union Set.empty
            )


findRoomDistancesAlongPath : Set String -> List Direction -> List Direction -> ( List Direction, Set String )
findRoomDistancesAlongPath rooms stack dirsToGo =
    case dirsToGo of
        [] ->
            ( stack, rooms )
        ...
        dir :: rest ->
            case stack of
                stackTop :: stackRest ->
                    if oppositeDirections dir stackTop then
                        findRoomDistancesAlongPath rooms stackRest rest

                    else
                        findRoomDistancesAlongPath (Set.insert (hashPath (dir :: stack)) rooms) (dir :: stack) rest

                [] ->
                    findRoomDistancesAlongPath (Set.insert (hashPath (dir :: stack)) rooms) (dir :: stack) rest


hashPath : List Direction -> String
hashPath dirs =
    List.map dirToChar dirs |> String.fromList
